...
@register_config_type("quantizer")
@dataclass(frozen=True, kw_only=True)
class QuantizerConfig(BaseConfig):
    """
    Configuration for time series quantization.
    """
    type: str = field(default="mean_std_bins")
    vocab_size: int = field(default=4096)
    num_features: int = field(default=1) # Used by some quantizers
    d_model: int = field(default=512) # Needed for VQ-VAE's embedding dim
