...
def test_encoder_only_forward_pass(basic_config):
    ...
    past_values = torch.randn(
        2, basic_config.context_length, basic_config.feature_size
    )
    ...
    assert output.logits.shape == (
        2,
        basic_config.context_length,
        basic_config.feature_size,
    )
...
def test_decoder_only_forward_pass(basic_config):
    ...
    future_values = torch.randn(
        2, basic_config.prediction_length, basic_config.feature_size
    )
    ...
    assert output.logits.shape == (
        2,
        basic_config.prediction_length,
        basic_config.feature_size,
    )
...
def test_batch_invariance(basic_config):
    ...
    past_values_batch1 = torch.randn(
        1, basic_config.context_length, basic_config.feature_size
    )
    future_values_batch1 = torch.randn(
        1, basic_config.prediction_length, basic_config.feature_size
    )

    past_values_batch4 = torch.randn(
        4, basic_config.context_length, basic_config.feature_size
    )
    future_values_batch4 = torch.randn(
        4, basic_config.prediction_length, basic_config.feature_size
    )
...
def test_data_types(basic_config):
    ...
    try:
        model(
            past_values=past_values.to(torch.float16),
            future_values=future_values.to(torch.float16),
        )
        model(
            past_values=past_values.to(torch.float32),
            future_values=future_values.to(torch.float32),
        )
    except Exception as e:
