...
from temporal.configs.transformer_config import TransformerTimeSeriesConfig, EmbeddingConfig
...
def test_preprocessor_basic_pass(preprocessor):
    ...
    assert output["hidden_states"].shape == (batch_size, seq_len, preprocessor.config.d_model)
...
def test_patched_preprocessor_padding(base_config):
    ...
    patched_config.value_embedding_config = EmbeddingConfig(type="patch", kwargs={"patch_size": 4})
