...
from temporal.configs.embedding_config import TimeSeriesPatchEmbeddingConfig, TimeSeriesValueEmbeddingConfig, SinusoidalPositionalEmbeddingConfig
...
def patched_decoder_config(decoder_config):
    ...
    config_dict["value_embedding_config"] = {
        "type": "patch", 
        "patch_size": 2, 
        "feature_size": decoder_config.feature_size
    }
...
def test_patched_decoder_forward_pass(patched_decoder):
    ...
    patch_size = patched_decoder.config.value_embedding_config.patch_size
...
def test_decoder_kv_caching_mechanism(decoder):
    # FIX: Put model in eval mode to disable layerdrop/dropout for a fair comparison
    decoder.eval()
