...
from temporal.models.transformer_model import TimeSeriesTransformer
...
def test_kv_cache_correctness(generation_model, generation_config):
    ...
    batch_size = 2
    past_values = torch.randn(batch_size, generation_config.context_length, generation_config.feature_size)
    future_values = torch.randn(batch_size, generation_config.prediction_length, generation_config.feature_size)

    # 1. Forward pass without cache (full sequence at once)
    with torch.no_grad():
        full_pass_output = model(past_values=past_values, future_values=future_values)
    ...
    encoder_output = model.encoder(input_values=past_values)
...
def test_model_serialization(generation_model):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        reloaded_model = TimeSeriesTransformer.from_pretrained(tmpdir)
        reloaded_model.eval()
        ...
        for p1, p2 in zip(model.parameters(), reloaded_model.parameters()):
            assert torch.equal(p1, p2), "Model parameters are not identical after reloading."
...
def test_generation_output_shape(generation_model, generation_config):
    ...
    # Generate a forecast
    with torch.no_grad():
        generated_sequence = model.generate(past_values=past_values)

    # Check the output shape
    expected_shape = (batch_size, generation_config.prediction_length, generation_config.feature_size)
    ...
    batch_size = 4
    past_values_b4 = torch.randn(batch_size, generation_config.context_length, generation_config.feature_size)
    with torch.no_grad():
        generated_sequence_b4 = model.generate(past_values=past_values_b4)
    expected_shape_b4 = (batch_size, generation_config.prediction_length, generation_config.feature_size)
