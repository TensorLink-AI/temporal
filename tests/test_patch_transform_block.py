...
def mock_builder():
    ...
    config = TransformerConfig(d_model=32, n_heads=4)
...
def test_patch_transform_block_forward_split_first(mock_builder):
    ...
    assert output.hidden_states.shape == input_tensor.shape
...
def test_patch_transform_block_invalid_order(mock_builder):
    ...
    with pytest.raises(ValueError, match="order must be one of 'split_first' or 'merge_first'"):
        PatchTransformBlock(
            builder=mock_builder,
            wrapped_block_type="default_encoder",
            expansion_factor=2,
...
def test_patch_transform_block_merge_first_invalid_expansion(mock_builder):
    ...
    with pytest.raises(ValueError, match="For 'merge_first' order with an MLP, expansion_factor must be 2."):
        PatchTransformBlock(
            builder=mock_builder,
            wrapped_block_type="default_encoder",
            expansion_factor=3,
...
def test_patch_transform_block_non_divisible_d_model_split_first(mock_builder):
    ...
    with pytest.raises(ValueError, match="d_model (33) must be divisible by expansion_factor (2)"):
        PatchTransformBlock(
            builder=mock_builder,
            wrapped_block_type="default_encoder",
            expansion_factor=2,
