...
import pytest
import torch
from temporal.modules.norm.revin import Revin
...
def test_revin_reversibility(sample_tensor):
    ...
    normalized_output = layer.normalize(sample_tensor)

    # 2. Denormalize the output
    denormalized_output = layer.denormalize(normalized_output)
...
def test_dynamic_revin_reversibility(sample_tensor):
    ...
    # 1. Normalize the tensor
    normalized_output, mean, stdev = layer.normalize(sample_tensor)

    # 2. Denormalize the output
    denormalized_output = layer.denormalize(normalized_output, mean, stdev)
...
def test_dynamic_revin_non_affine(sample_tensor):
    ...
    normalized, mean, stdev = layer.normalize(sample_tensor)
    denormalized = layer.denormalize(normalized, mean, stdev)
